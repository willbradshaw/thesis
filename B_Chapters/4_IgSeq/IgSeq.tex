...
is a method for computing a distance between any two repertoires on the basis of their V(D)J segment composition, based on the Euclidean distance between their respective V(D)J-abundance vectors. As such, it provides a pairwise distance metric which can be used to compare, cluster and visualise the relationship between different antibody repertoires. Two methods for visualising the RDI distances between pilot replicates are shown in \Cref{fig:igseq-pilot-rdi}: in \Cref{fig:igseq-pilot-rdi-dendro}, the replicates are arranged in a dendrogram by average-linkage clustering over their pairwise RDI distances, while for \Cref{fig:igseq-pilot-rdi-pcoa} the same distances were used to compute a principle co-ordinate analysis, % TODO: Cite this
...
\end{subfigure}
\caption[Repertoire dissimilarity index (RDI) analysis of pilot data]{\textbf{Repertoire dissimilarity index (RDI) analysis of pilot data:} (A) Dendrogram of pilot replicates produced through average-linkage clustering on pairwise RDI distances. (B) Principle co-ordinate analysis (PCoA) of pairwise RDI distances between pilot replicates, coloured by source individual.}
...
%
%% TODO: Describe origin, design and importance of gut microbiota study
%% TODO: Summary figure of gut microbiota study
%
%In order to investigate the relationship between the gut microbiota and antibody repertoire diversity, \igseq library-preps were performed on gut total RNA from ... % TODO: Discuss individuals included in LP, successful vs unsuccessful LPs, integrity scores, treatment groups
%
...
%
%The resulting dataset exhibits highly consistent behaviour to and including VDJ assignment and Change-O database construction, with \embed{_Figures/txt/gut-read-survival-init-min.txt}\,\% to \embed{_Figures/txt/gut-read-survival-init-max.txt}\,\% of reads surviving up to this stage in the pipeline. However, a substantially higher proportion of sequences (\embed{_Figures/txt/gut-read-survival-rel-loss-total.txt}\,\%) are lost during V-score filtering and clonotyping, with 
%
%\Cref{fig:igseq-gut-read-survival-all} shows the absolute and relative read survival for each of the sixty-four sequencing libraries throughout this process. As with the pilot dataset, the replicates show relatively consistent behaviour up  However, a somewhat % TODO: much?
% larger number of sequences (\embed{_Figures/txt/gut-read-survival-rel-loss-total.txt}\,\%, up to \embed{_Figures/txt/gut-read-survival-rel-loss-max.txt}\,\% per replicate) are lost during V-score filtering compared to the pilot dataset (\embed{_Figures/txt/pilot-read-survival-rel-loss-total.txt}\,\%, up to \embed{_Figures/txt/pilot-read-survival-rel-loss-max.txt}\,\% per replicate). This inconsistency is due to a greater preponderance in the ageing dataset of malformed, J-identity-lacking sequences, which in this case actually make up an absolute majority of unique sequences in the \program{pRESTO}-processed dataset (\Cref{fig:igseq-gut-functional-prop-pre}). After filtering on V-score, however, the functional composition of the ageing dataset is similar to that of the pilot data (\Cref{fig:gut-ageing-functional-prop-post}).
%
%
%\begin{figure}
%\centering
...
%\end{subfigure}
%\caption{Absolute (A) and relative (B) read survival during pre-processing of the \igseq gut-microbiota-transfer dataset, up to and including clonotyping.}
