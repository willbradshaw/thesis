###############################################################################
## FIGURE                                                                    ##
## Fitting rank/abundance of ageing clones to Zipf's law                     ##
###############################################################################
...
n_exclude <- c(6,3,2,5)
n_exclude_null <- rep(0, 32)
names(n_exclude) <- paste0("2-0", seq(3,6))
names(n_exclude_null) <- unique(tab$INDIVIDUAL)

clntab <- tab %>% group_by(INDIVIDUAL, SEQUENCE_INPUT, CLONE) %>%
  summarise() %>%
  compute_clntab() %>% compute_expected_frequencies(n_exclude_null) %>%
...
# TODO: Age-group colour scheme
palette <- gg_color_hue(32)
