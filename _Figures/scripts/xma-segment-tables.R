...
# Path to locus sequence
xma_locus_path <- "../_Data/locus/complete/xma_new.fasta"

# Path to segment sequences
vh_nt_path <- "../_Data/segments/xma/xma_new_vh_nt_annotated.fasta"
dh_nt_path <- "../_Data/segments/xma/xma_new_dh_nt.fasta"
jh_nt_path <- "../_Data/segments/xma/xma_new_jh_nt.fasta"
jh_aa_path <- "../_Data/segments/xma/xma_new_jh_aa.fasta"
ch_nt_path <- "../_Data/constant/xma/xma_new_ch_nt.fasta"

# Paths to RSS sequences
vh_rss_path <- "../_Data/rss/xma/xma_new_vh_rss.fasta"
dh_rss_path <- "../_Data/rss/xma/xma_new_dh_rss.fasta"
jh_rss_path <- "../_Data/rss/xma/xma_new_jh_rss.fasta"

# Output paths
for (segtype in paste0(c("v", "d", "j", "c"), "h")){
  assign(paste0("rangename_", segtype), paste0("xma-new-", segtype, "-ranges.tsv"))
  assign(paste0("tablename_", segtype), paste0("xma-new-", segtype, "-coords"))
}
rangename_all <- "xma-new-locus-ranges.tsv"
...
dh_nt <- readDNAStringSet(dh_nt_path)
jh_nt <- readDNAStringSet(jh_nt_path)
jh_aa <- readAAStringSet(jh_aa_path)
ch_nt <- readDNAStringSet(ch_nt_path)

# RSS
vh_rss <- readDNAStringSet(vh_rss_path)
dh_rss <- readDNAStringSet(dh_rss_path)
jh_rss <- readDNAStringSet(jh_rss_path)

# Locus
locus_unmasked <- readDNAStringSet(xma_locus_path)
...
for (n in 1:vh_n_tab){
  ...
  savetab(get(paste0("vh_tab_", n)), paste0(tablename_vh, "-", n))
}

# Save to latex tables
savetab(dh_tab_seg, paste0(tablename_dh, "-seg"))
savetab(dh_tab_rss5, paste0(tablename_dh, "-rss5"))
savetab(dh_tab_rss3, paste0(tablename_dh, "-rss3"))
savetab(jh_tab_seg, paste0(tablename_jh, "-seg"))
savetab(jh_tab_rss, paste0(tablename_jh, "-rss"))
savetab(ch_tab, tablename_ch)
