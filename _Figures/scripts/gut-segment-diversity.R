###############################################################################
## FIGURE                                                                    ##
## Plot V/J diversity spectra of gut clonal repertoires                      ##
###############################################################################
...
tab_path_grouped_group <- paste0("../_Data/changeo/spectra/gut-group_",
                                 segments, "-diversity-grouped_seqs-",
                                 seqset, "_copy-", copy, ".tsv")
tab_path_grouped_age <- paste0("../_Data/changeo/spectra/gut-age_",
                               segments, "-diversity-grouped_seqs-",
                               seqset, "_copy-", copy, ".tsv")
tab_path_solo_group <- paste0("../_Data/changeo/spectra/gut-group_",
                              segments, "-diversity-solo_seqs-",
                              seqset, "_copy-", copy, ".tsv")
tab_path_solo_age <- paste0("../_Data/changeo/spectra/gut-age_",
                            segments, "-diversity-solo_seqs-",
                            seqset, "_copy-", copy, ".tsv")
...
# Output paths
filename_base <- paste0("igseq-gut-", segments, "-diversity")
...
# Process grouped spectra for alpha/beta plots
...
g_alpha_group <- plot_diversity_alpha(tab_grouped_group, "GROUP") +
  ...
  theme(legend.title = element_text(margin = margin(r = 1, unit = "cm"))) +
  guides(colour = guide_legend(nrow = 2), fill = guide_legend(nrow = 2))
g_alpha <- gplot_grid(g_alpha_age, g_alpha_group)

# Rescaled beta plots
g_beta_age_scaled <- plot_diversity_beta_scaled(tab_grouped_age, "AGE_WEEKS") +
  scale_colour_manual(values = palette_age, name = "Age group (weeks)") +
  scale_fill_manual(values = palette_age, name = "Age group (weeks)") +
  ylim(c(0,0.35)) +
  theme(legend.title = element_text(margin = margin(r = 1, unit = "cm"))) +
  guides(colour = guide_legend(nrow = 2), fill = guide_legend(nrow = 2))
g_beta_group_scaled <- plot_diversity_beta_scaled(tab_grouped_group, "GROUP") +
  scale_colour_manual(values = palette, name = "Treatment group") +
  scale_fill_manual(values = palette, name = "Treatment group") +
  #ylim(c(0,0.35)) +
  theme(legend.title = element_text(margin = margin(r = 1, unit = "cm"))) +
  guides(colour = guide_legend(nrow = 2), fill = guide_legend(nrow = 2))
g_beta_scaled <- gplot_grid(g_beta_age_scaled, g_beta_group_scaled)
...
p_pos_age <- tibble(Q = qvals, XMIN = 1, XMAX = 2, XLAB = 1.5,
                    Y = c(120,80,60,55,40,30), SCALE = 1)
