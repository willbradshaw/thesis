###############################################################################
## FIGURE                                                                    ##
## Nothobranchius furzeri vs Xiphophorus maculatus VH-family comparison      ##
###############################################################################
...
# Path to V sequences
vh_nt_path_nfu <- "../_Data/segments/nfu/nfu_vh_nt.fasta"
vh_nt_path_xma <- "../_Data/segments/xma/xma_vh_nt.fasta"

# Configure output
filename_comb <- "combined-vh-families"
filename_tree <- "combined-vh-families-tree"
filename_map <- "combined-vh-families-map"
...
# Import V sequences
vh_nt_nfu <- readDNAStringSet(vh_nt_path_nfu)
names(vh_nt_nfu) <- paste0("nfu_", names(vh_nt_nfu))
vh_nt_xma <- readDNAStringSet(vh_nt_path_xma)
names(vh_nt_xma) <- paste0("xma_", names(vh_nt_xma))
vh_nt <- c(vh_nt_xma, vh_nt_nfu)
nv <- length(vh_nt)
...
vh_families <- vh_tbl_tree_raw %>% 
  filter(!is.na(label)) %>%
  mutate(family = sub("(.*)_IGH\\d?V(.*)-.*", "\\1_\\2", label)) %>%
  group_by(family) %>%
  count() %>%
...
vh_families <- vh_families %>%
  mutate(node = unlist(lapply(str_split(family, "_"), function(f)
    get_mrca(paste0(f[1], "_IGH\\d?V", f[2], "-"), vh_phylo))))
...
vh_tbl_tree_fam[copy_fill,]$family <- sub("(.*)_IGH\\d?V(.*)-.*", "\\1_\\2",
...
tre <- revts(ggtree(as.treedata(vh_tbl_tree_fam), aes(colour=family), 
  ...
  scale_colour_manual(values = vh_families$colour, breaks = vh_families$family[1:n_large],
                      name = "VH family") +
  coord_flip() + 
  scale_x_reverse(labels = function(x) x + 100, name = "Nucleotide sequence identity (%)",
                  limits = c(2, NA)) +
...
savefig(plot = plt, filename = filename_comb, device = "svg", 
        height = plot_height*1.5, ratio = plot_ratio_comb)
savefig(plot = plt, filename = filename_comb, device = "png", 
        height = plot_height*1.5, ratio = plot_ratio_comb)
