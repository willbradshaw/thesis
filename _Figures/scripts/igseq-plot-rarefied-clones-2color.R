...
clone_field <- "CLONE"
palette_exp <- c(colours_igseq[["exp_ageing"]], colours_igseq[["exp_pilot"]])
P <- 20
plot_height <- 15
plot_width <- 25

# Output paths
outpath <- "igseq-rarefied-clone-2colour"
...
r <- import_tsv(inpath, col = col) %>%
  mutate(ORGAN = factor(ifelse(EXPERIMENT=="gut", "gut", "whole-body"),
                        levels = c("whole-body", "gut")),
         EXPERIMENT = factor(EXPERIMENT, levels = c("pilot", "ageing", "gut")))
...
plot_nclones_means <- ggplot(r %>% filter(METRIC == "N_CLONES")) +
  geom_line(aes(x=SAMPLE_SIZE, y=MEAN, colour = ORGAN, 
                group = interaction(ORGAN,INDIVIDUAL,EXPERIMENT))) +
...
plot_nclones_sd <- plot_nclones_means +
  geom_ribbon(aes(x=SAMPLE_SIZE, ymin=MEAN-SD, ymax = MEAN + SD, 
                  fill = ORGAN,
                  group = interaction(ORGAN,EXPERIMENT,INDIVIDUAL)), alpha = 0.4) +
...
plot_p_means <- ggplot(r %>% filter(METRIC == paste0("P", P))) +
  geom_line(aes(x=SAMPLE_SIZE, y=MEAN, colour = ORGAN, 
                group = interaction(ORGAN,EXPERIMENT,INDIVIDUAL))) +
  scale_color_manual(values = palette_exp, name = "Sample type") +
  scale_x_continuous(breaks = seq(0, 10000, 2000), 
                     labels = function(x) x/1000, 
                     name = paste(x_title, "(×1000)")) +
...
plot_largeclones_pc <- ggplot(r %>% filter(METRIC == "PC_CLONES_SMALL")) +
  geom_line(aes(x=SAMPLE_SIZE, y=(1-MEAN), colour = ORGAN,
                group = interaction(ORGAN,EXPERIMENT,INDIVIDUAL))) +
  scale_color_manual(values = palette_exp, name = "Sample type") +
  scale_x_continuous(breaks = seq(0, 10000, 2000),
                     labels = function(x) x/1000,
