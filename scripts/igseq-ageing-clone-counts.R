...
clones_individual_max_pilot <- tab_cl_counts_pilot_indivs %>% pull(N) %>% max %>% 
  (function(x) ceiling(x/10^2)*10^2)
savetxt(clones_individual_min_pilot, outpath_clones_indiv_min_pilot)
savetxt(clones_individual_max_pilot, outpath_clones_indiv_max_pilot)
...
savetxt(cl_counts_kruskal$p.value %>% signif(2), outpath_counts_kruskal)
...
nclones_large <- tab_cl_size %>% filter(CLNCOUNT >= 5) %>% pull(N_PC) %>%
  sum %>% round(1)
savetxt(nclones_1count, outpath_pc_1count)
savetxt(nclones_small, outpath_pc_small)
...
savetxt(nseq_small_avg, outpath_seq_small)
...
savetxt(nseq_age_kruskal$p.value %>% signif(2), outpath_seq_small_kruskal)
...
ggsave(plot = g_nseq_abundant, filename = outpath_prop,
       device = "svg", units = "cm",
       height=15, width = 15*1.5)
...
savetxt(nclones_1rep, outpath_nclones_1rep)
savetxt(nclones_2rep, outpath_nclones_2rep)
