...
if (!exists("igh_nfu")){
  igh_nfu <- readDNAStringSet(inpath_locus)
}
...
vh_tab <- suppressMessages(read_tsv(inpath_vh))
ch_tab <- suppressMessages(read_tsv(inpath_ch))
jh_tab <- suppressMessages(read_tsv(inpath_jh))
dh_tab <- suppressMessages(read_tsv(inpath_dh))
...
sublocus <- subloci %>% as("GRanges")
sublocus_seqs <- getSeq(igh_nfu, sublocus)
...
ggsave(plot = g, filename = outpath, device = "svg", height=plot_height,
