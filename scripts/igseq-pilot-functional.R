...
tab_countfunc_filtered_melt <- tab_countfunc_filtered %>%
  ...
  ungroup() %>%
  melt(variable.name = c("VAR"), value.name = "VALUE", 
       id.var = c("FUNC_STATE", "FUNC_DESC")) %>% as_tibble() %>%
  group_by(VAR) %>%
  mutate(PC = VALUE / sum(VALUE) * 100)
...
nseq_filtered_other_pc <- tab_countfunc_filtered_summ %>% filter(!FUNCTIONAL, HAS_J) %>%
  pull(N_PC) %>% round(1)
savetxt(nseq_filtered_functional, outpath_nseq_functional_filtered)
savetxt(nseq_filtered_noj, outpath_nseq_noj_filtered)
savetxt(nseq_filtered_other, outpath_nseq_other_filtered)
savetxt(nseq_filtered_functional_pc, outpath_nseq_pc_functional_filtered)
savetxt(nseq_filtered_noj_pc, outpath_nseq_pc_noj_filtered)
savetxt(nseq_filtered_other_pc, outpath_nseq_pc_other_filtered)
...
savetxt(nseq_kept, outpath_nseq_kept_vscore)
savetxt(nreads_kept, outpath_nreads_kept_vscore)
