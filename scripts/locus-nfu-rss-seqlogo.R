...
# Get D/J sequences
vh_rss_nfu <- readDNAStringSet(inpath_vh)
dh_rss_nfu <- readDNAStringSet(inpath_dh)
jh_rss_nfu <- readDNAStringSet(inpath_jh)
...
ggsave(plot = plt_nfu_all, filename = outpath_all, device = "svg", units = "cm",
       height=plot_all_height, width = plot_all_height*plot_all_ratio)

plot_sep_height <- 30
plot_sep_ratio <- 2/3 # Width vs height

ggsave(plot = plt_nfu_sep, filename = outpath_sep, device = "svg", units = "cm",
