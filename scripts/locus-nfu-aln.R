...
bac_spread <- suppressMessages(read_tsv(inpath_bacs)) %>% # Import table
...
btab <- read_blast_table(inpath_aln, blast_fmt)
...
#------------------------------------------------------------------------------
# Prepare sublocus ranges
#------------------------------------------------------------------------------

nfu_ranges <- suppressMessages(read_tsv(inpath_ranges)) %>%
      mutate(sublocus = sub("(IGH\\d).*", "\\1", label)) %>%
        group_by(sublocus) %>% summarise(start = min(start), end=max(end))
...
g <- ggplot(summ_tab) + 
#  geom_rect(data = nfu_ranges, aes(xmin=start, xmax=end, ymin=0, ymax=Inf),
#            alpha=0.5, fill=colours[["LG"]]) +
...
ggsave(plot = g, filename = outpath, device = "svg", units = "cm",
